...
function wpcf7_textarea_form_tag_handler( $tag ) {
	...
	$html = sprintf('<span class="wpcf7-form-control-wrap dib wr-textarea %1$s"><textarea %2$s>%3$s</textarea>%4$s</span>',sanitize_html_class( $tag->name ), $atts,esc_textarea( $value ), $validation_error );
	// $html = sprintf('<textarea %2$s>%3$s</textarea>%3$s',sanitize_html_class( $tag->name ), $atts,esc_textarea( $value ), $validation_error );
