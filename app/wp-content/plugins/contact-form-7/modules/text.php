...
function wpcf7_text_form_tag_handler( $tag ) {
	...
	$html = sprintf('<span class="wpcf7-form-control-wrap dib wr-it %1$s"><input %2$s/>%3$s</span>',sanitize_html_class( $tag->name ), $atts, $validation_error );
	// $html = sprintf('<input %2$s />%3$s',sanitize_html_class( $tag->name ), $atts, $validation_error );
