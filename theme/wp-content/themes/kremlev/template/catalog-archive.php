...
?><section class="section catalog catalog-page">
	<div class="wr">
		<h1 class="title with-baner"><?php
			echo $cur_term->name;
			if( is_array( $banners ) && count( $banners ) ):
				foreach( $banners as $key=>$banner ):
					$banner_list = get_post_meta( $banner->ID, 'banner_list', !0 );
					if( is_array( $banner_list ) && count( $banner_list ) ):
						foreach( $banner_list as $key=>$item ):
							?><a href="<?php echo $item['banner_link']; ?>" class="a-d baner-in-category">
								<img src="<?php echo $item['banner_upload']; ?>" alt="<?php echo $item['title']; ?>">
							</a><?php
						endforeach;
					endif;
				endforeach;
